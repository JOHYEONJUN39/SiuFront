...
const ProfilePostPage = () => {
  ...
  useEffect(() => {
    GetPostUserId(`${userData.id}?page=${currentPage}`).then((response) => {
    ...
    <Container>
      <ProfileChangeCon>
        {posts.map((post) => (
          <PostCon key={post.id}>
            <img
              src={articleToThumbnail(post.article)}
              alt=""
            ...
            <PostArticle>
              {post.article.replace(/<img[^>]*>/g, "").replace(/<\/?p>/g, "")}
            </PostArticle>
            {post.tags && post.tags.length > 0 ? (
              <TagCon>
                {post.tags.map((tag, index) => (
                  <Tag
                    key={index}
                    onClick={() => handleTagSearch(tag.tag_name)}
                  >
                    {tag.tag_name}
                  </Tag>
                ))}
